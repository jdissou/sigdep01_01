...
import com.progenia.incubatis01_03.dialogs.EditerModeReglementDialog.ModeReglementRefreshEvent;
import com.progenia.incubatis01_03.dialogs.EditerModeReglementDialog.ModeReglementUpdateEvent;
import com.progenia.incubatis01_03.dialogs.EditerSequenceFacturationDialog;
import com.progenia.incubatis01_03.dialogs.EditerSequenceFacturationDialog.SequenceFacturationAddEvent;
import com.progenia.incubatis01_03.dialogs.EditerSequenceFacturationDialog.SequenceFacturationRefreshEvent;
import com.progenia.incubatis01_03.dialogs.EditerSequenceFacturationDialog.SequenceFacturationUpdateEvent;
import com.progenia.incubatis01_03.dialogs.EditerPeriodeFacturationDialog;
import com.progenia.incubatis01_03.dialogs.EditerPeriodeFacturationDialog.PeriodeFacturationAddEvent;
import com.progenia.incubatis01_03.dialogs.EditerPeriodeFacturationDialog.PeriodeFacturationRefreshEvent;
import com.progenia.incubatis01_03.dialogs.EditerPeriodeFacturationDialog.PeriodeFacturationUpdateEvent;
...
public class CorpusFacturationView extends OngletReferentielBase {
    ...
    */
    ...
    //ATTRIBUTS - tab43 - PeriodeFacturation
    private Tab tab43PeriodeFacturationTab = new Tab();
    private Grid<PeriodeFacturation> tab43PeriodeFacturationGrid = new Grid<>(); //Manually defining columns

    // @Autowired annotation provides the automatic dependency injection.
    @Autowired
    private PeriodeFacturationBusiness tab43PeriodeFacturationBusiness;
    private ArrayList<PeriodeFacturation> tab43PeriodeFacturationList = new ArrayList<PeriodeFacturation>();
    //For Lazy Loading
    //DataProvider<PeriodeFacturation, Void> tab43PeriodeFacturationDataProvider; //The second DataProvider type parameter defines how the provider can be filtered. In the example, the filter type is Void
    //For ListDataProvider for Advanced In-memory Data Handling
    private ListDataProvider<PeriodeFacturation> tab43PeriodeFacturationDataProvider; 

    /* Fields to filter items in PeriodeFacturation entity */
    private SuperTextField tab43NoPeriodeFilterTxt = new SuperTextField();
    private SuperTextField tab43LibellePeriodeFilterTxt = new SuperTextField();
    private SuperTextField tab43LibelleCourtPeriodeFilterTxt = new SuperTextField();
    ...
    */
    ...
    private void tab43SetupDataprovider()
    {
        try 
        {
            ...
            */
            ...
            //2- Setup the list 
            this.tab43PeriodeFacturationList = (ArrayList)this.tab43PeriodeFacturationBusiness.findAll();

            //3- Creates a new data provider backed by a collection
            this.tab43PeriodeFacturationDataProvider = DataProvider.ofCollection(this.tab43PeriodeFacturationList);
            
            //4- Make the tab43PeriodeFacturationDataProvider sorted by NoPeriode in ascending order
            this.tab43PeriodeFacturationDataProvider.setSortOrder(PeriodeFacturation::getNoPeriode, SortDirection.ASCENDING);
            
            //5- Set the data provider for this grid. The data provider is queried for displayed items as needed.
            this.tab43PeriodeFacturationGrid.setDataProvider(this.tab43PeriodeFacturationDataProvider);
                
        } 
        catch (Exception e) 
        {
            MessageDialogHelper.showAlertDialog("CorpusFacturationView.tab43SetupDataprovider", e.toString());
            e.printStackTrace();
        }
    } //private void tab43SetupDataprovider()
    ...
    private void tab43RefreshGrid()
    {
        ...
        */
        try 
        {
            ...
            //3 - Make the detailsDataProvider sorted by NoPeriode in ascending order
            this.tab43PeriodeFacturationDataProvider.setSortOrder(PeriodeFacturation::getNoPeriode, SortDirection.ASCENDING);
        ...
        {
            MessageDialogHelper.showAlertDialog("CorpusFacturationView.tab43RefreshGrid", e.toString());
            e.printStackTrace();
        }
    } //private void tab43RefreshGrid()
    ...
    private void tab43ConfigureGridWithFilters() {
        ...
        {
            //1 - Set properties of the grid
            this.tab43PeriodeFacturationGrid.addClassName("fichier-grid");
            this.tab43PeriodeFacturationGrid.getThemeNames().addAll(Arrays.asList("compact", "column-borders", "row-stripes", "wrap-cell-content"));
            
            this.tab43PeriodeFacturationGrid.setSizeFull(); //sets the grid size to fill the screen.
            
            //this.tab43PeriodeFacturationGrid.setSelectionMode(Grid.SelectionMode.SINGLE);
            this.tab43PeriodeFacturationGrid.setSelectionMode(Grid.SelectionMode.MULTI);
            
            //2 - Add columns to the empty table and set Data Properties to Columns - Width = 200 + 425 + 250 + 100 = 975
            Grid.Column<PeriodeFacturation> noPeriodeColumn = this.tab43PeriodeFacturationGrid.addColumn(PeriodeFacturation::getNoPeriode).setKey("NoPeriode").setHeader("N° Période").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("200px"); // fixed column
            Grid.Column<PeriodeFacturation> libellePeriodeColumn = this.tab43PeriodeFacturationGrid.addColumn(PeriodeFacturation::getLibellePeriode).setKey("LibellePeriode").setHeader("Libellé Période").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("375px"); // fixed column
            Grid.Column<PeriodeFacturation> libelleCourtPeriodeColumn = this.tab43PeriodeFacturationGrid.addColumn(PeriodeFacturation::getLibelleCourtPeriode).setKey("LibelleCourtPeriode").setHeader("Libellé Abrégé").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("275px"); // fixed column
            Grid.Column<PeriodeFacturation> isInactifColumn = this.tab43PeriodeFacturationGrid.addColumn(new ComponentRenderer<>(
                        secteurActivite -> {
                            Checkbox checkbox = new Checkbox();
            ...
            //3 - Add HeaderRow - Using text fields for filtering items
            HeaderRow filterRow = this.tab43PeriodeFacturationGrid.appendHeaderRow();

            //4 - Filtering In-memory Data - Filtering in the Grid Component
            // First filter
            this.tab43NoPeriodeFilterTxt.addValueChangeListener(event -> this.tab43ApplyFilterToTheGrid());
            this.tab43NoPeriodeFilterTxt.setValueChangeMode(ValueChangeMode.EAGER);

            filterRow.getCell(noPeriodeColumn).setComponent(this.tab43NoPeriodeFilterTxt);
            this.tab43NoPeriodeFilterTxt.setSizeFull();
            this.tab43NoPeriodeFilterTxt.setPlaceholder("Filtrer"); 
            this.tab43NoPeriodeFilterTxt.getElement().setAttribute("focus-target", "");            
            this.tab43NoPeriodeFilterTxt.setClearButtonVisible(true);  //DJ
            
            // Second filter
            this.tab43LibellePeriodeFilterTxt.addValueChangeListener(event -> this.tab43ApplyFilterToTheGrid());
            this.tab43LibellePeriodeFilterTxt.setValueChangeMode(ValueChangeMode.EAGER);

            filterRow.getCell(libellePeriodeColumn).setComponent(this.tab43LibellePeriodeFilterTxt);
            this.tab43LibellePeriodeFilterTxt.setSizeFull();
            this.tab43LibellePeriodeFilterTxt.setPlaceholder("Filtrer"); 
            this.tab43LibellePeriodeFilterTxt.getElement().setAttribute("focus-target", "");
            this.tab43LibellePeriodeFilterTxt.setClearButtonVisible(true); //DJ

            // Third filter
            this.tab43LibelleCourtPeriodeFilterTxt.addValueChangeListener(event -> this.tab43ApplyFilterToTheGrid());
            this.tab43LibelleCourtPeriodeFilterTxt.setValueChangeMode(ValueChangeMode.EAGER);

            filterRow.getCell(libelleCourtPeriodeColumn).setComponent(this.tab43LibelleCourtPeriodeFilterTxt);
            this.tab43LibelleCourtPeriodeFilterTxt.setSizeFull();
            this.tab43LibelleCourtPeriodeFilterTxt.setPlaceholder("Filtrer"); 
            this.tab43LibelleCourtPeriodeFilterTxt.getElement().setAttribute("focus-target", "");
            this.tab43LibelleCourtPeriodeFilterTxt.setClearButtonVisible(true); //DJ
        ...
        {
            MessageDialogHelper.showAlertDialog("CorpusFacturationView.tab43ConfigureGridWithFilters", e.toString());
            e.printStackTrace();
        }
    } //private void tab43ConfigureGridWithFilters() {
    ...
    private void tab43ApplyFilterToTheGrid() {
        ...
        {
            // Since this will be the only active filter, it needs to account for all values of my filter fields
            this.tab43PeriodeFacturationDataProvider.setFilter(item -> {
                boolean isNoPeriodeFilterMatch = true;
                boolean isLibellePeriodeFilterMatch = true;
                boolean isLibelleCourtPeriodeFilterMatch = true;
                boolean isInactifFilterMatch = true;

                if(!this.tab43NoPeriodeFilterTxt.isEmpty()){
                    isNoPeriodeFilterMatch = item.getNoPeriode().equals(this.tab43NoPeriodeFilterTxt.getValue());
                    //isNoPeriodeFilterMatch = item.getNoPeriode().toLowerCase(Locale.FRENCH).contains(this.tab43NoPeriodeFilterTxt.getValue().toLowerCase(Locale.FRENCH));
                }
                if(!this.tab43LibellePeriodeFilterTxt.isEmpty()){
                    isLibellePeriodeFilterMatch = item.getLibellePeriode().toLowerCase(Locale.FRENCH).contains(this.tab43LibellePeriodeFilterTxt.getValue().toLowerCase(Locale.FRENCH));
                }
                if(!this.tab43LibelleCourtPeriodeFilterTxt.isEmpty()){
                    isLibelleCourtPeriodeFilterMatch = item.getLibelleCourtPeriode().toLowerCase(Locale.FRENCH).contains(this.tab43LibelleCourtPeriodeFilterTxt.getValue().toLowerCase(Locale.FRENCH));
                }
                ...
                return isNoPeriodeFilterMatch && isLibellePeriodeFilterMatch && isLibelleCourtPeriodeFilterMatch && isInactifFilterMatch;
            });
        ...
        {
            MessageDialogHelper.showAlertDialog("CorpusFacturationView.tab43ApplyFilterToTheGrid", e.toString());
            e.printStackTrace();
        }
    } //private void tab43ApplyFilterToTheGrid() {
    ...
    private void configureTabs() {
        ...
        try 
        {
            ...
            this.tab43PeriodeFacturationTab.setLabel("Référentiel des Périodes de Facturation");
            ...
            this.tab43PeriodeFacturationGrid.setVisible(false); //At startup, set the second page visible, while the remaining are not
            ...
            this.tabsToPages.put(this.tab43PeriodeFacturationTab, this.tab43PeriodeFacturationGrid);
            this.tabsToPageNames.put(this.tab43PeriodeFacturationTab, "EditerPeriodeFacturationDialog");

            
            this.pages.add(this.tab10ModeReglementGrid, this.tab21SequenceFacturationGrid, this.tab43PeriodeFacturationGrid);        

            this.tabs.add(this.tab10ModeReglementTab, this.tab21SequenceFacturationTab, this.tab43PeriodeFacturationTab);
    ...
    protected void workingHandleAjouterClick(ClickEvent event) {
        try 
        {
            ...
            else if (this.selectedTab == this.tab43PeriodeFacturationTab)
            {
                EditerPeriodeFacturationDialog.getInstance().showDialog("Ajout de Période", ModeFormulaireEditerEnum.AJOUTERLOT, new ArrayList<PeriodeFacturation>(), this.tab43PeriodeFacturationList, "", this.uiEventBus);
            }
        } 
        catch (Exception e) 
        {
    ...
    protected void workingHandleModifierClick(ClickEvent event) {
        try 
        {
            ...
            else if (this.selectedTab == this.tab43PeriodeFacturationTab)
            {
                ...
                or several items in multi-selection mode.            
                */
                Set<PeriodeFacturation> selected = this.tab43PeriodeFacturationGrid.getSelectedItems();
                ...
                {
                    //Ouvre l'instance du Dialog EditerPeriodeFacturationDialog.
                    EditerPeriodeFacturationDialog.getInstance().showDialog("Modification de Période", ModeFormulaireEditerEnum.MODIFIER, new ArrayList<PeriodeFacturation>(selected), this.tab43PeriodeFacturationList, "", this.uiEventBus);
                }
            }
        } 
        catch (Exception e) 
        ...
        try 
        {
            ...
            else if (this.selectedTab == this.tab43PeriodeFacturationTab)
            {
                ...
                */
                ...
                {
                    EditerPeriodeFacturationDialog.getInstance().showDialog("Afficher détails Période", ModeFormulaireEditerEnum.AFFICHER, new ArrayList<PeriodeFacturation>(selected), this.tab43PeriodeFacturationList, "", this.uiEventBus);
                }
            }
        } 
        catch (Exception e) 
    ...
    @EventBusListenerMethod
    private void handlePeriodeFacturationAddEventFromEditorDialog(PeriodeFacturationAddEvent event) {
        //Handle PeriodeFacturation Add Event received from EditorView
        try 
        {
            //1 - Sauvegarder la modification dans le backend
            PeriodeFacturation newInstance = this.tab43PeriodeFacturationBusiness.save(event.getPeriodeFacturation());
        ...
        {
            MessageDialogHelper.showAlertDialog("CorpusFacturationView.handlePeriodeFacturationAddEventFromEditorDialog", e.toString());
            e.printStackTrace();
        }
    } //private void handlePeriodeFacturationAddEventFromEditorDialog(PeriodeFacturationAddEvent event) {
    
    @EventBusListenerMethod
    private void handlePeriodeFacturationUpdateEventFromEditorDialog(PeriodeFacturationUpdateEvent event) {
        //Handle PeriodeFacturation Udpdate Event received from EditorView
        try 
        {
            //1 - Sauvegarder la modification dans le backend
            PeriodeFacturation updateInstance = this.tab43PeriodeFacturationBusiness.save(event.getPeriodeFacturation());

            //2- Retrieving tab43PeriodeFacturationList from the database
            this.tab43RefreshGrid();
        } 
        catch (Exception e) 
        {
            MessageDialogHelper.showAlertDialog("CorpusFacturationView.handlePeriodeFacturationUpdateEventFromEditorDialog", e.toString());
            e.printStackTrace();
        }
    } //handlePeriodeFacturationUpdateEventFromEditorDialog(PeriodeFacturationUpdateEvent event) {
    
    @EventBusListenerMethod
    private void handlePeriodeFacturationRefreshEventFromEditorDialog(PeriodeFacturationRefreshEvent event) {
        //Handle PeriodeFacturation Cloee Event received from EditorView
        try 
        {
            //1 - Actualiser l'affichage du grid
            this.tab43PeriodeFacturationDataProvider.refreshAll();
        ...
        {
            MessageDialogHelper.showAlertDialog("CorpusFacturationView.handlePeriodeFacturationRefreshEventFromEditorDialog", e.toString());
            e.printStackTrace();
        }
    } //private void handlePeriodeFacturationRefreshEventFromEditorDialog(RefreshEvent event) {
            ...
            else if (this.selectedTab == this.tab43PeriodeFacturationTab)
            {
                ...
                else
                {
                    ...
                    ComponentEventListener<ClickEvent<Button>> yesClickListener = ev -> {
                        //Confirmer la suppression
                        //1 - Iterate Set Using For-Each Loop
                        for(PeriodeFacturation secteurActiviteItem : selected) {
                            this.tab43PeriodeFacturationBusiness.delete(secteurActiviteItem);
                        }            
            ...
            else if (this.selectedTab == this.tab43PeriodeFacturationTab)
            {
                ...
                else
                {
                    for(PeriodeFacturation secteurActiviteItem : selected) {
                        ...
                        //Enregistrer les mofdifications dans le backend
                        this.tab43PeriodeFacturationBusiness.save(secteurActiviteItem);

                    }   //for(PeriodeFacturation secteurActiviteItem : selected) {

                    //3- Retrieving tab43PeriodeFacturationList from the database
                    this.tab43RefreshGrid();

                    //4 - Annulation des sélections - indispensable
                    //this.tab43PeriodeFacturationGrid.deselectAll();
                } //if (selected.isEmpty() == true)
            }
        } 
        catch (Exception e) 
    ...
    protected void workingHandleImprimerClick(ClickEvent event) {
        try 
        {
            ...
            {
                this.execJasperReport("PeriodeFacturation", "Référentiel des Unités d'Œuvre", this.tab43PeriodeFacturationBusiness.getReportData());
            }
        } 
        catch (Exception e) 
        {
    ...
    public void customActivateMainToolBar()
    {
        try 
        {
            ...
            else if (this.selectedTab == this.tab43PeriodeFacturationTab)
            {
                ...
                //int fullSize = tab43PeriodeFacturationDataProvider.getItems().size(); // this is how you get the size of all items
                int filteredSize = this.tab43PeriodeFacturationDataProvider.size(new Query<>(this.tab43PeriodeFacturationDataProvider.getFilter()));

                if (filteredSize == 0) //if (this.tab43PeriodeFacturationList.size() == 0)
                {
                    this.btnModifier.setEnabled(false);
                    this.btnSupprimer.setEnabled(false);
    ...
    protected void applySelectedTabChanged()
    {
        try 
        {
            if ((this.selectedTab == this.tab10ModeReglementTab) || (this.selectedTab == this.tab21SequenceFacturationTab) || (this.selectedTab == this.tab43PeriodeFacturationTab))
            {
