...
import com.progenia.sigdep01_01.dialogs.EditerAbonnementServiceDialog;
import com.progenia.sigdep01_01.dialogs.EditerAbonnementServiceDialog.AbonnementServiceAddEvent;
import com.progenia.sigdep01_01.dialogs.EditerAbonnementServiceDialog.AbonnementServiceRefreshEvent;
import com.progenia.sigdep01_01.dialogs.EditerAbonnementServiceDialog.AbonnementServiceUpdateEvent;
...
public class MajAbonnementServiceView extends FichierMajListeBase<ZZZAbonnementService> {
    /*
    ...
    */
    ...
    static final String PAGE_TITLE = "Mise à jour des Abonnements de Service";
    
    /* Defines a new FormLayout. */
    private FormLayout formLayout = new FormLayout();
    
    /* Fields to filter properties in ZZZAbonnementService entity */
    private ComboBox<ZZZCentreIncubateur> cboCodeCentreIncubateurFilter = new ComboBox<>();
    private ComboBox<Instrument> cboNoInstrumentFilter = new ComboBox<>();
    
    /* Column in ZZZAbonnementService grid */
    /* Fields to filter items in ZZZAbonnementService entity */
    ...
    /* Column in ZZZAbonnementService grid */
    private Grid.Column<ZZZAbonnementService> noChronoColumn;
    private Grid.Column<ZZZAbonnementService> serviceFourniColumn;
    private Grid.Column<ZZZAbonnementService> dateAbonnementColumn;
    private Grid.Column<ZZZAbonnementService> dateDebutContratColumn;
    private Grid.Column<ZZZAbonnementService> dateFinContratColumn;
    private Grid.Column<ZZZAbonnementService> isClotureColumn;
    ...
    */
    ...
    private void initialize() {
        try 
        {
            ...
            this.reportName = "ZZZAbonnementService";
    ...
    private void setupDataprovider()
    {
        try 
        {
            ...
            */
            ...
            this.dataProvider.setSortOrder(ZZZAbonnementService::getCodeService, SortDirection.ASCENDING);
        } 
        catch (Exception e) 
        {
    ...
    @Override
    protected ArrayList<ZZZAbonnementService> workingFetchItems() {
        try 
        {
            //1 - Fetch the items
            return (ArrayList)this.abonnementServiceBusiness.getAbonnementService(this.centreIncubateurCible, this.cboNoInstrumentFilter.getValue());
        } 
        catch (Exception e) 
        {
    ...
    @Override
    protected void workingDeleteItem(ZZZAbonnementService abonnementServiceItem) {
        try 
        {
    ...
    private void configureComponents() {
        ...
        try 
        {
            ...
            this.cboNoInstrumentFilter.setWidth(150, Unit.PIXELS);
            ...
            this.cboNoInstrumentFilter.setItemLabelGenerator(Instrument::getLibelleInstrument); //setItemLabelGenerator() should return String directly
            this.cboNoInstrumentFilter.addValueChangeListener(event -> {
            ...
            this.formLayout.addFormItem(this.cboNoInstrumentFilter, "Instrument :").getStyle().set("--vaadin-form-item-label-width", FORM_ITEM_LABEL_WIDTH150);
    ...
    private void setComboBoxDataProvider() {
        ...
        try 
        {
            ...
            //this.cboNoInstrumentFilter.setItems(this.exerciceList);
        } 
        catch (Exception e) 
        {
    ...
    private void configureReadOnlyField() {
        try 
        {
            ...
            this.cboNoInstrumentFilter.setReadOnly(false); 
        } 
        catch (Exception e) 
        {
    ...
    private void configureGridWithFilters() {
        ...
        try 
        {
            ...
            this.noChronoColumn = this.grid.addColumn(ZZZAbonnementService::getNoChrono).setKey("NoChrono").setHeader("N° Abonnement").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("150px"); // fixed column
            this.serviceFourniColumn = this.grid.addColumn(ZZZAbonnementService::getLibelleService).setKey("ServiceFourni").setHeader("Service").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("200px"); // fixed column
            this.dateAbonnementColumn = this.grid.addColumn(ZZZAbonnementService::getDateAbonnementToString).setKey("DateAbonnement").setHeader("Date Abonnement").setTextAlign(ColumnTextAlign.CENTER).setFlexGrow(0).setWidth("150px"); // fixed column
            this.dateDebutContratColumn = this.grid.addColumn(ZZZAbonnementService::getDateDebutContratToString).setKey("DateDebutContrat").setHeader("Date Début Contrat").setTextAlign(ColumnTextAlign.CENTER).setFlexGrow(0).setWidth("150px"); // fixed column
            this.dateFinContratColumn = this.grid.addColumn(ZZZAbonnementService::getDateFinContratToString).setKey("DateFinContrat").setHeader("Date Fin Contrat").setTextAlign(ColumnTextAlign.CENTER).setFlexGrow(0).setWidth("150px"); // fixed column
    ...
    private void computeGridSummaryRow() {
        try 
        {
            ...
            ListDataProvider<ZZZAbonnementService> dataProvider = (ListDataProvider<ZZZAbonnementService>) this.grid.getDataProvider();
            int totalSize = dataProvider.getItems().size();
            DataCommunicator<ZZZAbonnementService> dataCommunicator = this.grid.getDataCommunicator();
            ...
            Supplier<Stream<ZZZAbonnementService>> streamSupplier = () -> dataProvider.fetch(new Query<>(0, totalSize,
                    dataCommunicator.getBackEndSorting(),
                    dataCommunicator.getInMemorySorting(),
                    dataProvider.getFilter()));
            
            this.totalSoldeDebiteur = streamSupplier.get().mapToLong(ZZZAbonnementService::getSoldeDebiteurOuverture).sum();
            this.totalSoldeCrediteur = streamSupplier.get().mapToLong(ZZZAbonnementService::getSoldeCrediteurOuverture).sum();
    ...
    */
    ...
    protected void workingHandleModifierClick(ClickEvent event) {
        try 
        {
            ...
            */
            Set<ZZZAbonnementService> selected = this.grid.getSelectedItems();
            ...
            else
            {
                ...
                EditerAbonnementServiceDialog.getInstance().showDialog("Modification d'Abonnement de Service Récurrent", ModeFormulaireEditerEnum.MODIFIER, new ArrayList<ZZZAbonnementService>(selected), this.targetBeanList, "", this.uiEventBus, this.serviceFourniBusiness);
            }
        } 
        catch (Exception e) 
        {
    ...
    private void handleAddEventFromEditorView(AbonnementServiceAddEvent event) {
        ...
        try 
        {
            ...
            ZZZAbonnementService newInstance = this.abonnementServiceBusiness.save(event.getAbonnementService());
    ...
    private void handleUpdateEventFromEditorView(AbonnementServiceUpdateEvent event) {
        ...
        {
            //1 - Sauvegarder la modification dans le backend
            ZZZAbonnementService updateInstance = this.abonnementServiceBusiness.save(event.getAbonnementService());
