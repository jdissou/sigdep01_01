...
public class EvenementIncubationLotView extends SaisieTransactionMaitreDetailsBase<ZZZEvenementIncubationLot, ZZZEvenementIncubationLotDetails> {
    ...
    */
    ...
    private ListDataProvider<Instrument> InstrumentDataProvider;     //Paramètres de Personnalisation ProGenia

    
    //Paramètres de Personnalisation ProGenia
    static final String PAGE_TITLE = "Saisie des Evénements d'incubation par Lot";
    ...
    */
    ...
    private void setGridBeanList() {
        try 
        {
            ...
            //Abandonnée, car crée une erreur - this.detailsDataProvider.setSortOrder(ZZZEvenementIncubationLotDetails::getNoInstrument, SortDirection.ASCENDING);
    ...
    private void configureGrid() {
        ...
        try 
        {
            ...
            this.libelleInstrumentColumn = this.grid.addColumn(ZZZEvenementIncubationLotDetails::getLibelleInstrument).setKey("LibelleInstrument").setHeader("Dénomination Instrument").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("300px");
    ...
    */
    ...
    protected void workingHandleDetailsClick(ClickEvent event) {
        try 
        {
            ...
            EditerEvenementIncubationLotDetailsDialog.getInstance().showDialog("Saisie des Instruments de l'Evénement d'Incubation", this.currentBean, this.detailsBeanList, this.uiEventBus, this.evenementIncubationLotDetailsBusiness, this.InstrumentBusiness, this.centreIncubateurCible);
        } 
        catch (Exception e) 
        {
    ...
    protected void workingHandleButtonOptionnel01Click(ClickEvent event) {
        //Réinitialiser Liste des Instruments
        ...
        try 
        {
            ...
            ComponentEventListener<ClickEvent<Button>> noClickListener = ev -> {
                ...
                MessageDialogHelper.showWarningDialog("Réinitialisation de la Liste des Instruments", "La Réinitialisation de la Liste des Instruments a été abandonnée.");
                //Cancel - Il ne vaut pas la peine d'appeler clear ou setValue (null) sur le composant (ce qui revient au même). Le ComboBox a déjà une valeur nulle
            };
            ...
            ComponentEventListener<ClickEvent<Button>> yesClickListener = ev -> {
                ...
                try 
                {
                    ...
                    MessageDialogHelper.showInformationDialog("Réinitialisation de la Liste des Instruments", "La Réinitialisation de la Liste des Instruments a été exécutée avec succès.");
                } 
                catch (Exception e) 
                {
            ...
            MessageDialogHelper.showYesNoDialog("Réinitialisation de la Liste des Instruments", "Désirez-vous valider la Saisie de l'Evénement d'incubation par Lot courant?. Cliquez sur Oui pour confirmer la validation.", yesClickListener, noClickListener);
        } 
        catch (Exception e) 
        {
    ...
    protected void workingHandleButtonOptionnel02Click(ClickEvent event) {
        ...
        try 
        {
            if (this.cboCodeCentreIncubateur.getValue() == null) {
                ...
                ComponentEventListener<ClickEvent<Button>> noClickListener = ev -> {
                    //Validation abandonnée
                    MessageDialogHelper.showWarningDialog("Actualisation de la Liste des Instruments", "L'Actualisation de la Liste des Instruments a été abandonnée.");
                    //Cancel - Il ne vaut pas la peine d'appeler clear ou setValue (null) sur le composant (ce qui revient au même). Le ComboBox a déjà une valeur nulle
                };
                    ...
                    try 
                    {
                        ...
                        for(Instrument Instrument:this.InstrumentCohorteList) {
                            ...
                            if (this.detailsBeanList.stream().noneMatch(p -> (p.getNoInstrument().equals(Instrument.getNoInstrument())))) {
                                //Ajout des enrégistrements dans ZZZEvenementIncubationLotDetails - Save it to the backend
                                //1 - Ajout des enrégistrements dans ZZZEvenementIncubationLotDetails
                        ...
                        if (this.isRowAdded) {
                            MessageDialogHelper.showInformationDialog("Actualisation de la Liste des Instruments", "L'Actualisation de la Liste des Instruments a été exécutée avec succès. Des Instruments ont été ajoutés.");
                        }
                        else {
                            MessageDialogHelper.showInformationDialog("Actualisation de la Liste des Instruments", "L'Actualisation de la Liste des Instruments a été exécutée avec succès. Aucun Instrument n'a été ajouté.");
                        }
                    } 
                    catch (Exception e) 
                    {
                ...
                // Affiche une boîte de confirmation demandant si l'utilisateur désire valider la Saisie de l'Evénement d'incubation par Lot courant.
                MessageDialogHelper.showYesNoDialog("Actualisation de la Liste des Instruments", "Désirez-vous valider la Saisie de l'Evénement d'incubation par Lot courant?. Cliquez sur Oui pour confirmer la validation.", yesClickListener, noClickListener);
            } //if (this.cboCodeCentreIncubateur.getValue() == null) {
        } 
        catch (Exception e) 
        {
    ...
    protected void workingExecuteValider() {
        ...
        try 
        {
            ...
            else if (this.detailsBeanList.size() == 0)
            {
                ...
                MessageDialogHelper.showWarningDialog("Validation de la Saisie de Transaction", "Aucun Instrument de Projet n'a été ajouté. Veuillez saisir des Instruments de projet.");
            }
