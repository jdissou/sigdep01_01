    ...
    */
    ...
    private void setGridBeanList() {
        try 
        {
            ...
            //Abandonnée, car crée une erreur - this.detailsDataProvider.setSortOrder(ZZZFacturationActeDetails::getNoInstrument, SortDirection.ASCENDING);
    ...
    */
    ...
    protected void workingHandleAfficherClick(ClickEvent event) {
        try 
        {
            ...
            AfficherFacturationActeDialog.getInstance().showDialog("Liste des Facturations de Service Récurrent par Instrument dans le Brouillard", this.uiEventBus, this.centreIncubateurCible, this.facturationActeBusiness, this.exerciceBusiness, this.utilisateurBusiness, this.centreIncubateurBusiness, this.InstrumentBusiness);
        } 
        catch (Exception e) 
        {
            ...
            if (this.cboCodeCentreIncubateur.getValue() == null) {
                ...
                ComponentEventListener<ClickEvent<Button>> yesClickListener = ev -> {
                    ...
                    try 
                    {
                        ...
                        if (this.isRowAdded) {
                            MessageDialogHelper.showInformationDialog("Actualisation de la Liste des Prestations", "L'Actualisation de la Liste des Prestations a été exécutée avec succès. Des Instruments ont été ajoutés.");
                        }
                        else {
                            MessageDialogHelper.showInformationDialog("Actualisation de la Liste des Prestations", "L'Actualisation de la Liste des Prestations a été exécutée avec succès. Aucun Instrument n'a été ajouté.");
                        }
                    } 
                    catch (Exception e) 
                    {
    ...
    private void performInitialiser(Boolean blnMajInstrument) {
        try 
        {
            ...
            for(PrestationDemande prestationDemande:this.prestationDemandeList) {
                //Ajout des enrégistrements dans ZZZFacturationActeDetails - Save it to the backend
                //1 - Ajout des enrégistrements dans ZZZFacturationActeDetails
    ...
    protected void workingExecuteValider() {
        ...
        try 
        {
            ...
            else if (!this.binder.writeBeanIfValid(this.currentBean)) //Blindage - Vérifier toutes les exigences liées au Binding
            {
                ...
                ComponentEventListener<ClickEvent<Button>> noClickListener = ev -> {
                    //Validation abandonnée
                    MessageDialogHelper.showWarningDialog("Validation de la Saisie de Transaction", "La Validation de la Saisie de la Facturation de Service Récurrent par Instrument courant a été abandonnée.");
                    //Cancel - Il ne vaut pas la peine d'appeler clear ou setValue (null) sur le composant (ce qui revient au même). Le ComboBox a déjà une valeur nulle
                };
                ...
                ComponentEventListener<ClickEvent<Button>> yesClickListener = ev -> {
                    ...
                    try 
                    {
                        ...
                        MessageDialogHelper.showInformationDialog("Validation de la Saisie de Transaction", "La Validation de la Saisie de la Facturation de Service Récurrent par Instrument courant a été exécutée avec succès.");
                ...
                // Affiche une boîte de confirmation demandant si l'utilisateur désire valider la Saisie de la Facturation de Service Récurrent par Instrument courant.
                MessageDialogHelper.showYesNoDialog("Validation de la Saisie de Transaction", "Désirez-vous valider la Saisie de la Facturation de Service Récurrent par Instrument courant?. Cliquez sur Oui pour confirmer la validation.", yesClickListener, noClickListener);
            } //if (this.exerciceCourant != null) {
        } 
        catch (Exception e) 
        {
