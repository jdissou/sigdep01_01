...
public class EditerRubriqueComptabilisationDialog extends BaseEditerReferentielDetailFormDialog<RubriqueComptabilisation> {
    ...
    */
    ...
    ComboBox<TypeInstrument> cboCodeTypeInstrument = new ComboBox<>();
    //ComboBox<TypeInstrument> cboCodeTypeInstrument = new ComboBox<>("Type de Instrument");
    ...
    // Show Dialog
    public void showDialog(String dialogTitle, Rubrique parentBean, ArrayList<RubriqueComptabilisation> targetBeanList, ArrayList<RubriqueComptabilisation> referenceBeanList, EventBus.UIEventBus uiEventBus, TypeInstrumentBusiness typeInstrumentBusiness, CompteBusiness compteBusiness) {
        ...
        try 
        {
            ...
            Collections.sort(this.targetBeanList, Comparator.comparing(RubriqueComptabilisation::getCodeTypeInstrument));
    ...
    private void configureComponents() {
        ...
        try 
        {
            ...
            this.cboCodeTypeInstrument.setWidth(400, Unit.PIXELS);
            this.cboCodeTypeInstrument.addClassName(COMBOBOX_LEFT_LABEL);
            
            // Choose which property from TypeInstrument is the presentation value
            this.cboCodeTypeInstrument.setItemLabelGenerator(TypeInstrument::getLibelleTypeInstrument);
            this.cboCodeTypeInstrument.setRequired(true);
            this.cboCodeTypeInstrument.setRequiredIndicatorVisible(true);
            //???this.cboCodeTypeInstrument.setLabel("TypeInstrument");
            //???this.cboCodeTypeInstrument.setId("person");
            
            this.cboCodeTypeInstrument.setClearButtonVisible(true);
            //Add Filtering
            this.cboCodeTypeInstrument.setAllowCustomValue(true);
            this.cboCodeTypeInstrument.setPreventInvalidInput(true);
            
            this.cboCodeTypeInstrument.addValueChangeListener(event -> {
                if (event.getValue() != null) {
                    //BeforeUpdate CodeTypeInstrument (CIF): Contrôle de Inactif
                    if (event.getValue().isInactif() == true) {
                        MessageDialogHelper.showWarningDialog("Erreur de Saisie", "Le Type de Instrument choisi est actuellement désactivé. Veuillez en saisir un autre.");
                        //Cancel
                        this.cboCodeTypeInstrument.setValue(event.getOldValue());
                    } //if (event.getValue() != null) {
                }
            });
            ...
            */
            ...
            this.cboCodeTypeInstrument.addCustomValueSetListener(event -> {
                this.cboCodeTypeInstrument_NotInList(event.getDetail(), 50);
            });
            ...
            */
            ...
            Label lblTypeInstrumentValidationStatus = new Label();
            this.binder.forField(this.cboCodeTypeInstrument)
                .asRequired("La Saisie du Type de Instrument est requise. Veuillez sélectionner un Type de Instrument")
                .bind(RubriqueComptabilisation::getTypeInstrument, RubriqueComptabilisation::setTypeInstrument); 
            ...
            */
            ...
            //this.formLayout.add(this.cboCodeTypeInstrument, this.cboNoCompteProduits, this.chkIncrementerCompteProduits);
            //4 - Alternative
            this.formLayout.addFormItem(this.cboCodeTypeInstrument, "Type de Instrument :").getStyle().set("--vaadin-form-item-label-width", FORM_ITEM_LABEL_WIDTH200);
    ...
    protected void configureReadOnlyField() {
        try 
        {
            this.cboCodeTypeInstrument.setReadOnly(this.isContextualFieldReadOnly); 
    ...
    private void setComboBoxDataProvider() {
        ...
        try 
        {
            this.cboCodeTypeInstrument.setDataProvider(this.typeInstrumentDataProvider);
    ...
    private void cboCodeTypeInstrument_NotInList(String strProposedVal, int intMaxFieldLength)
    {
        //Ajoute un nouveau Type de Instrument en entrant un libellé dans la zone de liste modifiable CodeTypeInstrument.
        String strNewVal = strProposedVal;

        try 
        {
            if (SecurityService.getInstance().isAccessGranted("EditerTypeInstrumentDialog") == true) {
                //Java Lambda Implementation of ComponentEventListener<ClickEvent<Button>>
                ComponentEventListener<ClickEvent<Button>> noClickListener = ev -> {
                    //Ajout non accompli
                    MessageDialogHelper.showWarningDialog("Erreur de Saisie", "La Saisie du Type de Instrument est requise. Veuillez en saisir un.");
                ...
                ComponentEventListener<ClickEvent<Button>> yesClickListener = ev -> {
                    ...
                    //Affiche une boîte de message et ajuste la longueur de la valeur introduite dans la zone de liste modifiable cboCodeTypeInstrument.
                    ...
                    //Ouvre l'instance du Dialog EditerTypeInstrumentDialog.
                    EditerTypeInstrumentDialog.getInstance().showDialog("Ajout de Type de Instrument", ModeFormulaireEditerEnum.AJOUTERCIF, new ArrayList<TypeInstrument>(), this.typeInstrumentList, finalNewVal, this.uiEventBus, this.compteBusiness);
                };

                // Affiche une boîte de confirmation demandant si l'utilisateur désire ajouter un nouveau Type Instrument.
                MessageDialogHelper.showYesNoDialog("Le Type de Instrument '" + strNewVal + "' n'est pas dans la liste.", "Désirez-vous ajouter un nouveau Type de Instrument?. Cliquez sur Oui pour confirmer l'ajout.", yesClickListener, noClickListener);
            }
            else {
                MessageDialogHelper.showWarningDialog("Erreur de Saisie", "La Saisie du Type de Instrument est requise. Veuillez en saisir un.");
            } //if (SecurityService.getInstance().isAccessGranted("EditerTypeInstrumentDialog") == true) {
        ...
        {
            MessageDialogHelper.showAlertDialog("EditerRubriqueComptabilisationDialog.cboCodeTypeInstrument_NotInList", e.toString());
            e.printStackTrace();
        }
    } //private void cboCodeTypeInstrument_NotInList(String strProposedVal, int intMaxFieldLength)
    
    @EventBusListenerMethod
    private void handleTypeInstrumentAddEventFromDialog(EditerTypeInstrumentDialog.TypeInstrumentAddEvent event) {
        //Handle Ajouter TypeInstrument Add Event received from Dialog
        //Ajouté à cause du CIF
        try 
        {
            //1 - Sauvegarder la modification dans le backend
            TypeInstrument newInstance = this.typeInstrumentBusiness.save(event.getTypeInstrument());
            ...
            */
            ...
            this.typeInstrumentDataProvider.getItems().add(newInstance);
            this.typeInstrumentDataProvider.refreshAll();
            this.cboCodeTypeInstrument.setValue(newInstance);
        } 
        catch (Exception e) 
        {
            MessageDialogHelper.showAlertDialog("EditerRubriqueComptabilisationDialog.handleTypeInstrumentAddEventFromDialog", e.toString());
            e.printStackTrace();
        }
    } //private void handleTypeInstrumentAddEventFromDialog(TypeInstrumentAddEvent event) {
    ...
    protected boolean workingIsPrimaryKeyAndBeanExtraCheckValidated()
    {
        ...
        try
        {
            ...
            currentKeyValue.setCodeTypeInstrument(this.cboCodeTypeInstrument.getValue().getCodeTypeInstrument());

            if (this.referenceBeanList.stream()
                    .anyMatch(p -> (p != this.currentBean) && 
                            (p.getRubriqueComptabilisationId().equals(currentKeyValue)))) {
                blnCheckOk = false;
                this.cboCodeTypeInstrument.focus();
    ...
    private class SortByCodeTypeInstrument implements Comparator<RubriqueComptabilisation> {
        // Used for sorting in ascending order of
        // getCodeTypeInstrument()
        public int compare(RubriqueComptabilisation a, RubriqueComptabilisation b)
        {
            return a.getCodeTypeInstrument().compareTo(b.getCodeTypeInstrument());
        }
    }
}
