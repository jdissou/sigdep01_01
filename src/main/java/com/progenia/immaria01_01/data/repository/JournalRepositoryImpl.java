...
/*
    ...
    JournalRepositoryImpl extends JournalRepositoryCustom.
*/
...
public class JournalRepositoryImpl implements JournalRepositoryCustom {
    ...
    public List<JournalPojo> getReportData() {
        List<JournalPojo> results = new ArrayList<>();
        ...
        try {
            ...
            results = this.entityManager.createNativeQuery("{call ReqSpEtatJournal()}", "JournalPojo")           
               .getResultList();
            return results;
        }
        catch (Exception e) 
        {
            MessageDialogHelper.showAlertDialog("JournalRepositoryImpl.getReportData", e.toString());
            e.printStackTrace();
            return null;
        }
    }
