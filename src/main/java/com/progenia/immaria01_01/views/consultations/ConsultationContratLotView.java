...
public class ConsultationContratLotView extends ConsultationSimpleBase<ContratLot> {
    /*
    ...
    */
    ...
    private ArrayList<ContratLotDetails> contratLotDetailsList = new ArrayList<ContratLotDetails>();
    ...
    final static String PAGE_TITLE = "Consultation des Services Récurrents par Lot";
    ...
    */
    ...
    private void setupDataprovider()
    {
        try 
        {
            ...
            */
            ...
            this.dataProvider.setSortOrder(ContratLot::getNoChrono, SortDirection.DESCENDING);
        } 
        catch (Exception e) 
        {
    ...
    private void configureGridWithFilters() {
        ...
        try 
        {
            ...
            Grid.Column<ContratLot> noChronoColumn = this.grid.addColumn(ContratLot::getNoChrono).setKey("NoChrono").setHeader("N° Contrat").setTextAlign(ColumnTextAlign.CENTER).setFlexGrow(0).setWidth("125px"); // fixed column
            ...
            //Grid.Column<ContratLot> dateContratColumn = this.grid.addColumn(new LocalDateRenderer<>(ContratLot::getDateContrat, DateTimeFormatter.ofLocalizedDate(FormatStyle.SHORT))).setKey("DateContrat").setHeader("Date Contrat").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("175px"); // fixed column
    ...
    private Boolean isTransactionCancelable(ContratLot contratLotItem) {
        try 
        {
            //return (true);
    ...
    @Override
    protected void workingProcedAnnulerTransactionItem(ContratLot contratLotItem) {
        try 
        {
            if (this.isTransactionCancelable(contratLotItem) == true) {
                //1 - Mise à jour de SaisieValidée dans ContratLot
                contratLotItem.setSaisieValidee(false);

                //2 - Suppression dans AbonnementService
                ...
                for(ContratLotDetails contratLotDetails:this.contratLotDetailsList) {
                    Long noAbonnement = contratLotDetails.getNoAbonnement();
                    Optional<AbonnementService> abonnementServiceOptional = this.abonnementServiceBusiness.findById(noAbonnement);
    ...
    @Override
    protected Long workingGetCurrentNoTransaction(ContratLot contratLotItem) {
        try 
        {
