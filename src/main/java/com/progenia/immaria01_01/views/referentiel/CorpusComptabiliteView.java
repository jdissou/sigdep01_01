...
import com.progenia.immaria01_01.dialogs.EditerOperationComptableDialog;
import com.progenia.immaria01_01.dialogs.EditerOperationComptableDialog.OperationComptableAddEvent;
import com.progenia.immaria01_01.dialogs.EditerOperationComptableDialog.OperationComptableRefreshEvent;
import com.progenia.immaria01_01.dialogs.EditerOperationComptableDialog.OperationComptableUpdateEvent;
import com.progenia.immaria01_01.dialogs.EditerExerciceDialog;
import com.progenia.immaria01_01.dialogs.EditerExerciceDialog.ExerciceAddEvent;
import com.progenia.immaria01_01.dialogs.EditerExerciceDialog.ExerciceRefreshEvent;
import com.progenia.immaria01_01.dialogs.EditerExerciceDialog.ExerciceUpdateEvent;
...
public class CorpusComptabiliteView extends OngletReferentielBase {
    ...
    */
    ...
    final static String PAGE_TITLE = "Corpus de Comptabilité";
    final static String CACHED_SELECTED_TAB_INDEX = "CorpusComptabiliteViewSelectedTab";
    ...
    //ATTRIBUTS - tab34 - Journal
    private Tab tab34JournalTab = new Tab();
    private Grid<Journal> tab34JournalGrid = new Grid<>(); //Manually defining columns

    // @Autowired annotation provides the automatic dependency injection.
    @Autowired
    private JournalBusiness tab34JournalBusiness;
    private ArrayList<Journal> tab34JournalList = new ArrayList<Journal>();
    //For Lazy Loading
    //DataProvider<Journal, Void> tab34JournalDataProvider; //The second DataProvider type parameter defines how the provider can be filtered. In the example, the filter type is Void
    //For ListDataProvider for Advanced In-memory Data Handling
    private ListDataProvider<Journal> tab34JournalDataProvider; 
    ...
    /* Fields to filter items in Journal entity */
    ...
    */
    ...
    private void tab34SetupDataprovider()
    {
        try 
        {
            ...
            */
            ...
            this.tab34JournalDataProvider.setSortOrder(Journal::getCodeJournal, SortDirection.ASCENDING);
    ...
    private void tab34ConfigureGridWithFilters() {
        ...
        try 
        {
            ...
            Grid.Column<Journal> codeJournalColumn = this.tab34JournalGrid.addColumn(Journal::getCodeJournal).setKey("CodeJournal").setHeader("Code Journal").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("200px"); // fixed column
            Grid.Column<Journal> libelleJournalColumn = this.tab34JournalGrid.addColumn(Journal::getLibelleJournal).setKey("LibelleJournal").setHeader("Libellé Journal").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("275px"); // fixed column
            Grid.Column<Journal> referenceJournalColumn = this.tab34JournalGrid.addColumn(Journal::getReferenceJournal).setKey("ReferenceJournal").setHeader("Référence Journal").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("175px"); // fixed column

            Grid.Column<Journal> journalColumn = this.tab34JournalGrid.addColumn(new ComponentRenderer<>(
                        journal -> {
            ...
            Grid.Column<Journal> isInactifColumn = this.tab34JournalGrid.addColumn(new ComponentRenderer<>(
                        journal -> {
                            Checkbox checkbox = new Checkbox();
    ...
    private void tab45ConfigureGridWithFilters() {
        ...
        try 
        {
            ...
            //2 - Add columns to the empty table and set Data Properties to Columns - Width = 200 + 425 + 250 + 100 = 975
            Grid.Column<Exercice> noExerciceColumn = this.tab45ExerciceGrid.addColumn(new NumberRenderer<>(Exercice::getNoExercice, NumberFormat.getIntegerInstance(Locale.FRENCH))).setKey("NoExercice").setHeader("Code Exercice").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("200px"); // fixed column
            Grid.Column<Exercice> intituleExerciceColumn = this.tab45ExerciceGrid.addColumn(Exercice::getIntituleExercice).setKey("IntituleExercice").setHeader("Nom Exercice").setTextAlign(ColumnTextAlign.START).setFlexGrow(0).setWidth("325px"); // fixed column
    ...
    protected void workingHandleAjouterClick(ClickEvent event) {
        try 
        {
            ...
            else if (this.selectedTab == this.tab34JournalTab)
            {
                ...
                EditerJournalDialog.getInstance().showDialog("Ajout de Journal", ModeFormulaireEditerEnum.AJOUTERLOT, new ArrayList<Journal>(), this.tab34JournalList, "", this.uiEventBus, this.tab34CompteBusiness);
            }
    ...
    protected void workingHandleModifierClick(ClickEvent event) {
        try 
        {
            ...
            else if (this.selectedTab == this.tab34JournalTab)
            {
                ...
                or several items in multi-selection mode.            
                */
                Set<Journal> selected = this.tab34JournalGrid.getSelectedItems();
                ...
                {
                    EditerJournalDialog.getInstance().showDialog("Modification de Journal", ModeFormulaireEditerEnum.MODIFIER, new ArrayList<Journal>(selected), this.tab34JournalList, "", this.uiEventBus, this.tab34CompteBusiness);
                }
            }
            else if (this.selectedTab == this.tab45ExerciceTab)
            {
        ...
        try 
        {
            ...
            else if (this.selectedTab == this.tab34JournalTab)
            {
                ...
                */
                ...
                {
                    //Ouvre l'instance du Dialog EditerJournalDialog.
                    EditerJournalDialog.getInstance().showDialog("Afficher détails Journal", ModeFormulaireEditerEnum.AFFICHER, new ArrayList<Journal>(selected), this.tab34JournalList, "", this.uiEventBus, this.tab34CompteBusiness);
                }
            }
            else if (this.selectedTab == this.tab45ExerciceTab)
            {
    ...
    private void handleJournalAddEventFromEditorDialog(JournalAddEvent event) {
        //Handle Journal Add Event received from EditorView
        try 
        {
            //1 - Sauvegarder la modification dans le backend
            Journal newInstance = this.tab34JournalBusiness.save(event.getJournal());
    ...
    private void handleJournalUpdateEventFromEditorDialog(JournalUpdateEvent event) {
        //Handle Journal Udpdate Event received from EditorView
        try 
        {
            //1 - Sauvegarder la modification dans le backend
            Journal updateInstance = this.tab34JournalBusiness.save(event.getJournal());
    ...
    private void handleJournalRefreshEventFromEditorDialog(JournalRefreshEvent event) {
        //Handle Journal Cloee Event received from EditorView
        try 
        {
            //1 - Actualiser l'affichage du grid
            ...
            else if (this.selectedTab == this.tab34JournalTab)
            {
                ...
                else
                {
                    ...
                    ComponentEventListener<ClickEvent<Button>> yesClickListener = ev -> {
                        //Confirmer la suppression
                        //1 - Iterate Set Using For-Each Loop
                        for(Journal journalItem : selected) {
    ...
    protected void workingHandleImprimerClick(ClickEvent event) {
        try 
        {
            ...
            {
                this.execJasperReport("Journal", "Référentiel des Journaux", this.tab34JournalBusiness.getReportData());
            }
            else if (this.selectedTab == this.tab45ExerciceTab)
            {
